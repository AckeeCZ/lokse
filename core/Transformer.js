...
var androidTransformer = {
    ...
    transformPluralsValues: function (key, values) {
        var ouput = '    <plurals name="' + key + '">' + EOL;

        for (var i = 0; i < values.length; i++) {
            var normalizedValue = values[i].getValue().replace(/%newline%/gi, "\\n");
            normalizedValue = normalizedValue.replace(/'/gi, "\\'");
            normalizedValue = normalizedValue.replace(/%([sdf])/gi, '%#$$$1');
            normalizedValue = normalizedValue.replace(/&/gi, "&amp;");
            normalizedValue = normalizedValue.replace(/\u00A0/gi, "\\u00A0");
            ...
            ouput += '        <item quantity="' + values[i].getPluralKey() + '">' + normalizedValue + '</item>' + EOL;
...
module.exports = {'ios': iOSTransformer, 'android': androidTransformer, 'web': jsonTransformer, 'dart': dartTransformer, 'dartTemplate': dartTemplateTransformer, '.net': dotNetTransformer };
