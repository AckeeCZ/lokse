...
export default class Update extends Base {
  ...
  static flags = {
    ...
    cols: flags.string({char: 'c', name: 'cols', description: 'Translation columns'}),
    type: flags.string({char: 't', name: 'type', description: 'Type (key_web, key_android, key_ios)'}),
  ...
  async run() {
    ...
    const sheetID = flags.id ?? get(this.config, 'localization.sheet_id', false)
    ...
    if (type === 'key_web') {
      ...
      translations.map(item => {
        const filePath = path.join(process.cwd(), dir, item.toLowerCase() + '.json')
